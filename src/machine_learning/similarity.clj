...
(defn euclidean-similarity-for-features [map1 map2 features]
  (double (/ 1 (+ 1 (euclidean-distance-for-features map1 map2 features)))))

(defn tokenize [genres-string]
  (set (str/split (str/trim genres-string) #"(-| )")))

(defn cant-tokenize [item1 item2 feature]
  (and (empty? (feature item1)) (empty? (feature item2))))
...
(defn overal-similarity [item1 item2 numeric-features tokenizable-feature]
  (let [euclidean-similarity (euclidean-similarity-for-features item1 item2 numeric-features)
        genre-similarity (similarity-for-tokenizable-feature item1 item2 tokenizable-feature)]
    (+ euclidean-similarity genre-similarity)))

(defn find-top-similar [item items features tokenizable-feature num-of-matches]
  (let [filtered-items (filter #(not= (:remote_id item) (:remote_id %)) items)
        test (reverse (sort-by #(overal-similarity % item features tokenizable-feature) filtered-items))]
